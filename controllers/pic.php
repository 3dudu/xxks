...
class Pic extends IController
{
	...
	{
		$upObj = new IUpload();
		$upObj->setDir($this->app->config['upload'].'/editor/'.date("Ymd"));
	...
	{
		//配置参数
		$mixData = IFilter::act(IReq::get('pic'));
		if(!$mixData){
		...
		if($mixData)
		{
			$img = urldecode($mixData);
			if(stripos($img,'http')!==false){
				$w =  IFilter::act(IReq::get('w'),'int');
				$h =  IFilter::act(IReq::get('h'),'int');
				//http 304缓存
				if(isset($_SERVER['HTTP_IF_NONE_MATCH']) && $_SERVER['HTTP_IF_NONE_MATCH'] == md5($img.$w.$h))
				{
					header("HTTP/1.1 304 Not Modified");
					exit;
				}
				...
				if($w && $h)
				{
					$thumbSrc = Thumb::get($img,$w,$h);
				}
				//原图形式
				else
				{
					$thumbSrc = Thumb::get($img);
				}
				...
				if(isset($widthData[1]) && isset($heightData[1]))
				{
					...
					$w =  IFilter::act(IReq::get('w'),'int');
					$h =  IFilter::act(IReq::get('h'),'int');
					$thumbSrc = Thumb::get($mixData,$w,$h);
				}
