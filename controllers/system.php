    ...
    */
	...
	function save_conf()
	{
		...
		switch($form_index)
		{
			case "base_conf":
			{
				...
				if(isset($_FILES['wxcode']['name']) && $_FILES['wxcode']['name']!='')
				{
				    $uploadDir = IWeb::$app->config['upload'].'/logo';
					$uploadObj = new PhotoUpload($uploadDir);
					$uploadObj->setIterance(false);
					$photoInfo = $uploadObj->run();
					if(isset($photoInfo['wxcode']['img']) && file_exists($photoInfo['wxcode']['img']))
					{
						$_POST['wxcode'] = $photoInfo['wxcode']['img'];
					}
				}
			}
			break;
			...
			case "other_conf":
			{
				...
				if(isset($_FILES['startimg']['name']) && $_FILES['startimg']['name']!='')
				{
				    $uploadDir = IWeb::$app->config['upload'].'/startimg';
					$uploadObj = new PhotoUpload($uploadDir);
					$uploadObj->setIterance(false);
					$photoInfo = $uploadObj->run();
					if(isset($photoInfo['startimg']['img']) && file_exists($photoInfo['startimg']['img']))
					{
						IFile::xcopy($uploadDir.'/'.$photoInfo['startimg']['name'],$uploadDir.'/default.jpg');
						$_POST['startimg'] = $photoInfo['startimg']['dir'].'default.jpg';
					}
				}
			}
			break;
	...
	function admin_edit()
	{
		...
		if($id)
		{
			...
			if(!$adminRow['area']){
				$adminRow["province"]='320000';
				$adminRow["city"]='321300';
				$adminRow["area"]='321301';
			}
	...
	function clearCache()
	{
		...
		$defaultCache = new ICache('redis','API');
    ...
    public function area_update()
    {
    	...
    	$new_area_id = IFilter::act(IReq::get('new_area_id'));
    	...
    	else
    	{
			...
			if($new_area_id)
    		{
				if($areasDB->getObj('area_id='.$new_area_id)){
					die(JSON::encode(array('isSuccess' => false,'data' => '地区id已存在')));
				}
    			$updateData['area_id'] = $new_area_id;
    		}
			...
			die(JSON::encode(array('isSuccess' => true,'data' => $updateData)));
    	}
    }
	...
	function banner_update()
	{
	    ...
	    $bannerObj->del("type = '$banner_type'");
	    if($config_slide)
	    {
