...
class Comment extends IController implements adminAuthorization
{
	...
	function suggestion_list()
	{
        ...
        $query->join  = 'left join member AS b ON a.user_id = b.user_id left join user_company as uc on a.user_id=uc.user_id left join company as c on c.id=uc.com_id';
        $query->where  = $where;
        $query->fields = 'a.*,b.true_name,b.mobile,c.name as c_name,c.id as c_id';
