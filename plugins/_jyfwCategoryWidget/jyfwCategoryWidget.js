 ...
 */
function jyfw_categoryWidget(param)
{
	param._self = this;
	if(param['id'])
	{
		...
		param.id    = "_jyfwCategoryButton";
	}
	if(param['idArea'])
	{
		param.idArea    = param['idArea'];
	}else{
		param.idArea= "__jyfwcategoryBox";
	}
	...
	//生成分类按钮数据
	this.createjyfwCategory = function(categoryObj)
	{
		if(!categoryObj)
		{
			return;
		...
		buttonTemplate += '<button class="btn btn-default" type="button" id="_jyfwcatButton<%=templateData.id%>" onclick="return confirm(\'确定删除此分类？\',function(){$(\'#_jyfwcatButton<%=templateData.id%>\').parent().remove();})">';
		...
		for(var item in categoryObj)
		{
			...
			var jyfwCategoryHtml = template.compile(buttonTemplate)({'templateData':item});
			dataArea.append(jyfwCategoryHtml);
		}
	}

	//弹出分类界面进行选择
	this.selectjyfwCategory = function()
	{
		...
		{
			var urlValue = creatUrl('block/jyfw_category/type/checkbox/id/'+param.id);
		}
		else
		{
			var urlValue = creatUrl('block/jyfw_category/type/radio/id/'+param.id);
		}
		...
		art.dialog.open(urlValue,{
			...
			ok:function(iframeWin, topWin)
			{
				...
				{
					param._self.createjyfwCategory(categoryObject);
	...
	if(param.default)
	{
		this.createjyfwCategory(param.default);
		if(param.afterCallback)
		{
	...
	//绑定UI按钮入口
	$(document).on("click","[name='"+param.id+"']",this.selectjyfwCategory);
}