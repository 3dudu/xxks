...
class _userInfo extends pluginBase
{
	...
	public function userLoginAct()
	{
    	...
    	$remember   = IFilter::act(IReq::get('remember'));
		...
		{
			if(isset($userRow['user_company']) && ($userRow['user_company']['is_lock'] || $userRow['user_company']['c_is_del'])){
				return "所属企业不存在或账号被锁，请联系企业管理员";
			}
	...
	public function initUser()
	{
		...
		if(!isset($controller->user) || !isset($this->user['user_company'])){
	...
	{
		if($userRow = Api::getUserByToken()){
			$userRow['user_id'] = $userRow['id'];
			if(!isset($userRow['user_company'])){
				$userRow = Api::refrushUserCache($userRow['user_id']);
			}
			return $userRow;
		}
		...
		if($user['username'] && $user['user_pwd'] && $userRow = self::isValidUser($user['username'],$user['user_pwd']))
		{
			if(isset($userRow['user_company']) && ($userRow['user_company']['is_lock'] || $userRow['user_company']['c_is_del'])){
				return null;
			}
	...
	public static function isValidUser($login_info,$password)
	{
		...
		if($login_info)
		{
    		...
    		if($userRow && ($userRow['password'] == $password))
    		{
				...
				$user_company = $user_companyDB->getObj("uc.user_id=".$userRow['user_id']." and uc.com_id=c.id and uc.is_del=0","uc.*,c.name as company_name,c.hy_id,c.is_lock,c.is_del as c_is_del");
				if($user_company){
					$userRow['user_company']=$user_company;
					$userRow['gw_id']=$user_company['gw_id'];
				}else{
					$userRow['user_company']=false;
					$userRow['gw_id']=0;
				}
				return $userRow;
    		}
