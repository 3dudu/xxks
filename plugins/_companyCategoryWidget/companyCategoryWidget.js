 ...
 */
function hy_categoryWidget(param)
{
	param._self = this;
	if(param['id'])
	{
		...
		param.id    = "_companyCategoryButton";
	}
	if(param['idArea'])
	{
	...
	//生成分类按钮数据
	this.createcompanyCategory = function(categoryObj)
	{
		if(!categoryObj)
		{
			return;
		...
		buttonTemplate += '<button class="btn btn-default" type="button" id="_companycatButton<%=templateData.id%>" onclick="return confirm(\'确定删除此分类？\',function(){$(\'#_companycatButton<%=templateData.id%>\').parent().remove();})">';
		...
		for(var item in categoryObj)
		{
			...
			var companyCategoryHtml = template.compile(buttonTemplate)({'templateData':item});
			dataArea.append(companyCategoryHtml);
		}
	}

	//弹出分类界面进行选择
	this.selectcompanyCategory = function()
	{
		...
		{
			var urlValue = creatUrl('block/company_category/type/checkbox/id/'+param.id);
		}
		else
		{
			var urlValue = creatUrl('block/company_category/type/radio/id/'+param.id);
		}
		...
		art.dialog.open(urlValue,{
			...
			ok:function(iframeWin, topWin)
			{
				...
				{
					param._self.createcompanyCategory(categoryObject);
	...
	if(param.default)
	{
		this.createcompanyCategory(param.default);
		if(param.afterCallback)
		{
	...
	//绑定UI按钮入口
	$(document).on("click","[name='"+param.id+"']",this.selectcompanyCategory);
}