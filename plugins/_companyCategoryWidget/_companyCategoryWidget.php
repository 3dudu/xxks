 ...
 * @file _companyCategoryWidget.php
 * @brief 商品分类视图插件
 * @author nswe
 * @version 4.4
 * @date 2016/3/26 9:47:48
 */
class _companyCategoryWidget extends pluginBase
{
	//是否加载过了JS
	...
	public function reg()
	{
		plugin::reg("companyCategoryWidget",$this,"showWidget");
		plugin::reg("onBeforeCreateAction@block@company_category",function(){
			self::controller()->company_category = function(){$this->view("companyCategory");};
		});
	}
	...
	public function showWidget($param)
	{
		...
		$param['table'] = isset($param['table'])     ? $param['table']     : "hy_category";
		...
		$param['categoryParentData'] = company_class::categoryParentStruct($param['categoryData']);
		...
		if(self::$isJsLoad == false)
		{
			...
			$widgetRoot   = IUrl::creatUrl("")."plugins/_companyCategoryWidget/companyCategoryWidget.js";
...
jQuery(function()
{
	_companyCategoryWidget = new hy_categoryWidget({$param});
});
</script>
OEF;
	}
