 ...
 * @file _memberCategoryWidget.php
 * @brief 商品分类视图插件
 * @author nswe
 * @version 4.4
 * @date 2016/3/26 9:47:48
 */
class _memberCategoryWidget extends pluginBase
{
	//是否加载过了JS
	...
	public function reg()
	{
		plugin::reg("memberCategoryWidget",$this,"showWidget");
		plugin::reg("onBeforeCreateAction@block@member_category",function(){
			self::controller()->member_category = function(){$this->view("memberCategory");};
		});
	}
	...
	public function showWidget($param)
	{
		...
		$param['table'] = isset($param['table'])     ? $param['table']     : "gw_category";
		...
		if(self::$isJsLoad == false)
		{
			...
			$widgetRoot   = IUrl::creatUrl("")."plugins/_memberCategoryWidget/memberCategoryWidget.js";
...
jQuery(function()
{
	_memberCategoryWidget = new gw_categoryWidget({$param});
});
</script>
OEF;
	}
