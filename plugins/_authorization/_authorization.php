 ...
 * 4,com  （企业管理）  继承comAuthorization接口
 *
 * 使用方法：
...
class _authorization extends pluginBase
{
	 ...
	 * 控制器名称(controller) @ 动作名称(action)
	 */
	private static $adminShareComAction = array
	(
		'goods@spec_edit',
		'goods@spec_update',
		'goods@member_price',
	...
	private static $comAction = array();
	...
	public function reg()
	{
		...
		plugin::reg("clearComadmin",$this,"clearComadmin");
		plugin::reg("checkUserRights",$this,"checkUserRights");
	}
	...
	public function clearComadmin()
	{
		ISafe::clear('com_id');
		ISafe::clear('com_admin_name');
    	ISafe::clear('com_admin_pwd');
	}
	...
	public function authorizationCheck($actionId)
	{
		...
		//用户权限判断
		else if($controller instanceof comAuthorization)
		{
			self::checkComRights($actionId);
		}
	}
	...
	public static function getAdmin()
	{
		$admin = array(
			...
			'admin_areaName' => ISafe::get('admin_areaName'),
		);
		...
		if($adminRow = self::isValidAdmin($admin['admin_name'],$admin['admin_pwd']))
		{
			...
			$admin['level']  = $adminRow['level'];
			$admin['province']  = $adminRow['province'];
			$admin['city']  = $adminRow['city'];
			$admin['area']  = $adminRow['area'];
			return $admin;
		}
		return null;
	}
	 ...
	 */
	public static function getComadmin()
	{
		$comadmin = array(
			'com_admin_name' => ISafe::get('com_admin_name'),
			'com_admin_pwd'  => ISafe::get('com_admin_pwd'),
		);

		if($comadmin = self::isValidCom($comadmin['com_admin_name'],$comadmin['com_admin_pwd']))
		{
			return $comadmin;
		}
		return null;
	 ...
	 * @param $actionId string 动作ID
	 */
	public static function checkComRights($actionId)
	{
		$object       = IWeb::$app->getController();
		$controllerId = $object->getId();

		//1,针对独立配置的action检测
		if(isset(self::$comAction[$controllerId."@".$actionId]) && method_exists(__CLASS__,self::$comAction[$controllerId."@".$actionId]))
		{
			call_user_func(array(__CLASS__,self::$comAction[$controllerId."@".$actionId]));
			return;
		}
		//2,其余action检测
		else
		{
			$comRow = self::getComadmin();
			if(!$comRow)
			{
				$object->redirect('/systemcom/index');
				exit;
			}
	 ...
	 * @param array or false
	 */
	private static function isValidCom($login_info,$password)
	{
		$login_info = IFilter::act($login_info);
		$password   = IFilter::act($password);

		$comaObj = new IModel('company_admin');
		$where     = "name = '{$login_info}' and is_del = 0 and is_lock = 0";
		$comaRow = $comaObj->getObj($where);

		if($comaRow && ($comaRow['password'] == $password))
		{
			$comObj = new IModel('company');
			$comRow = $comObj->getObj("id =".$comaRow['com_id']." and is_del = 0 and is_lock = 0");
			if($comRow){
				$comaRow['company']=$comRow;
				return $comaRow;
			}
		}
		return false;
