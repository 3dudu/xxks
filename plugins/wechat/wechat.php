...
<Nonce><![CDATA[%s]]></Nonce>
    ...
    {
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_get(sprintf("https://api.weixin.qq.com/cgi-bin/user/info?access_token=%s&openid=%s&lang=zh_CN",$this->access_token,$openId));
    ...
    // 生成带场景值的二维码
    public function dwMasterCode($action_info,$scene_id,$type = 'QR_LIMIT_SCENE')
    {
        if (!$this->access_token && !$this->checkAuth()) return false;
        ...
        $postData = json_encode(array(
                ...
                'action_name' => $type,
                'action_info' => array(
                        'scene' => array('scene_id' => $scene_id,'action' => $action_info),
                    )
            ));
        $url = 'https://api.weixin.qq.com/cgi-bin/qrcode/create?access_token='.$this->access_token;
        $res = $this->http_post($url,$postData);
        $res = json_decode($res,true);
        if(!isset($res['url']) || $res['url'] == '') return;
        return $res['url'];
    }
    ...
    public function createMenu($data){
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post(self::API_URL_PREFIX.self::MENU_CREATE_URL.'access_token='.$this->access_token,json_encode($data,JSON_UNESCAPED_UNICODE));
        if ($result)
        {
            $json = json_decode($result,true);
            if (!$json || !empty($json['errcode'])) {
    ...
    *type：使用的下载方式
    */
    public function downloadMedia($media_id,$is_video=false,$file_name=false,$savedir=false){
        if (!$this->access_token && !$this->checkAuth()) return false;
//        $url = "https://api.weixin.qq.com/cgi-bin/media/get?access_token=" . $this->access_token . "&media_id=" . $media_id;
        // MD！！！！微信接口变了，2017-04-09
        $url = "http://file.api.weixin.qq.com/cgi-bin/media/get?access_token=" . $this->access_token . "&media_id=" . $media_id;
        $save_dir=$savedir?$savedir:'';

        $filename=$file_name?$file_name:time();
        $type=1;
        $ext=".jpg";//以jpg的格式结尾
