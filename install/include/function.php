...
function install_sql()
{
	...
	//检测SQL安装文件
	$sql_file = ROOT_PATH.'./install/base.sql';
	if(!file_exists($sql_file))
	{
		showProgress(array('isError' => true,'message' => '安装的SQL文件'.basename($sql_file).'不存在'));
	}

	$addon_sql_file = array();
	if(in_array('area',$install_type))
	{
		$addon_sql = ROOT_PATH.'./install/area_data.sql';
		if(!file_exists($addon_sql))
		{
			showProgress(array('isError' => true,'message' => '地区数据的SQL文件'.basename($addon_sql).'不存在'));
		}
		$addon_sql_file[] = $addon_sql;
	}

	if(in_array('app',$install_type))
	{
		$addon_sql = ROOT_PATH.'./install/app.sql';
		if(!file_exists($addon_sql))
		{
			showProgress(array('isError' => true,'message' => '应用数据的SQL文件'.basename($addon_sql).'不存在'));
		}
		$addon_sql_file[] = $addon_sql;
	}

	if(in_array('shop',$install_type))
	{
		$addon_sql = ROOT_PATH.'./install/shop.sql';
		if(!file_exists($addon_sql))
		{
			showProgress(array('isError' => true,'message' => '商城数据的SQL文件'.basename($addon_sql).'不存在'));
		}
		$addon_sql_file[] = $addon_sql;
	}
	...
	foreach($addon_sql_file as $val){
		parseSQL($val,$mysql_link);
	}

	$mysql_link->query("SET FOREIGN_KEY_CHECKS = 1;");

	//插入管理员数据
	$adminSql = 'insert into `'.$db_pre.'admin` (`admin_name`,`password`,`role_id`,`create_time`,`province`,`city`,`area`,`level`) values ("'.$admin_user.'","'.md5($admin_pwd).'",0,"'.date('Y-m-d H:i:s').'",320000,321300,321301,1)';
