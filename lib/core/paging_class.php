...
class IPaging
{
	...
	public function getPageBarSizeUrl()
	{
		$url   = IUrl::getUri();
		$parse = parse_url($url);
		...
		if(isset($parse['path']))
		{
			$parse['path'] = preg_replace("%/pagesize/\d+/%","/",$parse['path']);
		}
		...
		$url .= strpos($url,"?") === false ? "?pagesize=" : "&pagesize=";
		return $url;
	}
	...
	private function pcPagerHtml($url='', $attrs='')
	{
		...
		//6,统计数据
		$result .= "<li><a>当前第{$this->index}页/共{$this->totalpage}页,共{$this->rows}条</a></li>";
		$result .= "<li><a>每页</a></li>";
		$basesizeUrl = $url ? $url  : $this->getPageBarSizeUrl();
		$basesizeUrl = IFilter::act($basesizeUrl,'text');
		$s = [20,50,100,200,500];
		foreach($s as $size){
			$attr = str_replace('[pagesize]',$size,$attrs);
			$href = $basesizeUrl.($flag ? $size : "");
			$active = $size==$this->pagesize ? "active" : "";
			$result .= "<li class='{$active}'><a href='{$href}' {$attr}>".$size."</a></li>";
		}
