...
class IFilter
{
	...
	public static function act($str,$type = 'string',$limitLen = false)
	{
		...
		else
		{
			...
			switch($type)
			{
				...
				case "text2":
					return self::text2($str,$limitLen);
					break;
	...
	public static function text2($str,$limitLen = false)
	{
		$str = trim($str);
		$str = self::limitLen($str,$limitLen);
	 ...
	 */
	public static function addSlash($str,$s=false)
	{
		if(is_array($str))
		{
			$resultStr = array();
			foreach($str as $key => $val)
			{
				$resultStr[$key] = self::addSlash($val,$s);
			}
			return $resultStr;
		}
		else
		{
			return addslashes(self::word($str,$s));
		}
	}
	 ...
	 * @return string
	 */
	public static function word($str,$s=false)
	{
