...
class IClient
{
	...
	{
		$encryptKey = IWeb::$app->config['encryptKey'];
		return isset($_SERVER['HTTP_USER_AGENT']) && strpos($_SERVER['HTTP_USER_AGENT'], $encryptKey) !== false;
	}

	public static function clientVersion()
	{
		if(isset($_SERVER['HTTP_USER_AGENT'])){
			$APP_UA = IWeb::$app->config['APP_UA'];

			if ( preg_match("/(". $APP_UA . ")/i", strtolower($_SERVER['HTTP_USER_AGENT']),$result))
			{
				$cur_ua = $result[1];
				if(preg_match("/".$cur_ua."\/([\d\._]+)/i", strtolower($_SERVER['HTTP_USER_AGENT']),$result)){
					return $result[1];
				}
			}
		}
	 ...
	 * 判断是否是客户端访问
	 */
	public static function isAppCLient()
	{
		$APP_UA = IWeb::$app->config['APP_UA'];

		if (isset($_SERVER['HTTP_USER_AGENT']) && preg_match("/(". $APP_UA . ")/i", strtolower($_SERVER['HTTP_USER_AGENT'])))
		{
			return true;
		}
		return false;
