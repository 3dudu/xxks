...
class IUrl
{
	...
	const UrlForamtName	= 'format';
	const UrlVersionName	= 'ver';
	...
	public static function getInfo($key)
	{
		$arr = array(
			...
			'format'    => self::UrlForamtName,
			'ver'    => self::UrlVersionName,
		);
	...
	{
		return isset($_SERVER['SCRIPT_NAME']) ? basename($_SERVER['SCRIPT_NAME']) : 'api.php';
	}
	...
	public static function getRefRoute($isPathInfo = true)
	{
		...
		{
			if(($host & $_SERVER['HTTP_REFERER']) == $host && $isPathInfo == true)
			{
	...
	public static function imgFullUrl($html)
	{
		$html = preg_replace_callback('/<img[^>]*src="(.*?)"[^>]*>/', function($matches){
			$imgsrc = $matches[1];
			if(strpos($imgsrc,'http')!==0){
				$imgsrc = self::getHost().$imgsrc;
			}
			$ext = substr($imgsrc, -3);
			if(!in_array($ext,['png','jpg','peg'])){
				$imgsrc .= '#t.png';
			}
			return str_replace($matches[1],$imgsrc,$matches[0]);
		}, $html);
		$html = preg_replace_callback('/<img[^>]*src=\'(.*?)\'[^>]*>/', function($matches){
			$imgsrc = $matches[1];
			if(strpos($imgsrc,'http')!==0){
