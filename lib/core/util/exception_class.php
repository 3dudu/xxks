...
class IException extends Exception
{
	...
	public static function phpException($excep)
	{
		...
		$_controller = IWeb::$app->controller;
		
		if(!$_controller || $_controller->getDataFormat()=='json'){
			header("content-type:application/json;charset=".IWeb::$app->charset);
			if(self::$debugMode == false)
		{
			echo JSON::encode(array("ret_code"=>999999,"ret_msg"=>$excep->getMessage()));
		}else{
			echo JSON::encode(array("ret_code"=>999999,"ret_msg"=>$excep->getMessage(),"ret_error"=>array(
				'excep'=>$logArray,
				'traceData'=>$traceData,
			)));
		}
...
class IError
{
	...
	public static function show($httpNum = 403,$errorData = "")
	{
		...
		if($_controller->getDataFormat()=='json'){
			header("content-type:application/json;charset=".IWeb::$app->charset);
			echo JSON::encode(array("ret_code"=>$httpNum,"ret_msg"=>$defaultError));
			exit;
		}
