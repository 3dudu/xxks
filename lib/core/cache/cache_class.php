...
class ICache
{
	 ...
	 */
	public function __construct($cacheType = '',$cacheName = '')
	{
		...
		$this->logObj = new IFileLog("cache/".date("y-m-d").".log");
		...
		switch($this->cacheType)
		{
			...
			if ( !extension_loaded('memcache') ) {
				$this->logObj->write('memcache extension not loaded!');
				$this->cache = new IFileCache($cacheName);
			}else{
			...
			if ( !extension_loaded('redis') ) {
				$this->logObj->write('redis extension not loaded!');
				$this->cache = new IFileCache($cacheName);
			}else{
				$this->cache = new IRedisCache(array('prefix'=>$cacheName,'expire'=>$this->expire));
			}
			break;
	...
	{
		$data = $this->cache->get($key,$expire);
		if($data)
		{
	...
	{
		return $this->cache->flush($prefix);
	}
