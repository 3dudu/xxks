...
class WxCrawler
{
  ...
  public function crawByUrl($url)
  {
    ...
    list($content_html, $images) = $this->contentHandle($content);
    $newimags = array();
    if($basicInfo['cover']){
      $newimags[0] = $basicInfo['cover'];
      if(count($images)>1){
        $r = rand(1,3);
        if($r==2){
            $newimags[1] = $images[0];
    ...
    return array_merge($basicInfo,['content_html' => $content_html,'images' => $newimags]);
  }
  /**
  ...
  public function contentHandle($content)
  {
      //
        ...
        /** @var  带图片html文本 */
        preg_match_all('/data-src="(.*?)"/', $content, $images_m);
        if($images_m && count($images_m)>1){
          $i = 0;
           foreach($images_m[1] as $val){
             if($i++>2)break;
             $urll = IUrl::creatUrl("/pic/thumb/img/".urlencode($val));
             $urll = substr($urll,1);
             $images[] = $urll;
            }
        }
        ...
        $content_html = preg_replace_callback('/data-src="(.*?)"/', function($matches){
          return 'alt="图片" src='.IUrl::creatUrl("/pic/thumb/img/".urlencode($matches[1]));
        }, $content_html);
        $content_html = str_replace("visibility: hidden;", '',$content_html);
        $content_html = str_replace("!important", '',$content_html);

        $content_html = preg_replace_callback('/<iframe.*><\/iframe>/', function($matches){
          return '';
        }, $content_html);
      ...
      /*
        $content_html = preg_replace_callback('/alt="[^"]*"/', function($matches){
          return '';
        }, $content_html);
*/
        /** @var  无图html文本 */
  ...
  { 
    ...
    /** 获取作者 */
    preg_match('/<meta name="author" content="([^>]*?)" \/>/s', $content, $matchAuthor);
    if(!empty($matchAuthor[1])) $basicInfo['author'] = $matchAuthor[1];

    preg_match('/<meta property="og:title" content="([^>]*?)" \/>/s', $content, $matchTitle);
    if(!empty($matchTitle[1])) $basicInfo['title'] = $matchTitle[1];

    /** 文章类型 */
    preg_match('/<span id="copyright_logo" class="rich_media_meta rich_media_meta_text meta_tag_text">(.*?)<\/span>/s', $content, $matchType);
    if(!empty($matchType[1])) $basicInfo['copyright_stat'] = $matchType[1];
    if($basicInfo['cover']){
      $urll = IUrl::creatUrl("/pic/thumb/img/".urlencode($basicInfo['cover']));
      $urll = substr($urll,1);
      $basicInfo['cover'] = $urll;
    }
    return $basicInfo;
  }
  /**
