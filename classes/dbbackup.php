...
class DBBackup
{
	...
	function ppp($fileName){
		...
		while(!feof($fhandle))
		{
			$lstr = fgets($fhandle);     //获取指针所在的一行数据
			$lstr = trim($lstr);
			if(isset($lstr[0]) && count($lstr)>0){
				if(preg_match('/[A-Z]/i', $lstr)){
					$pid1 = 0;
					$curl = 1;
					$mix = true;
					continue;
				}
				if(preg_match('/^\d{2}$/', $lstr)){
					if($curl==1){
						if($mix){
							$pid2 = $pid1;
						}else{
				...
				if($mix && $curl==1){
					$categoryObj->setData(array(
						'name'=>$lstr,
						'parent_id'=>$pid1,
					));
					$tpid = $categoryObj->add();
					$mix = false;
				}
