...
return array(
    ...
    'getOrderRecycleList' => array(
        'query'=>array(
            ...
            'fields'  => 'o.id as oid,d.name as dname,p.name as pname,o.order_no,o.accept_name,o.pay_status,o.pay_type,o.distribution_status,u.username,o.create_time,o.status',
            'where'   => 'if_del = 1',
            'page'    => IReq::get('page') ? IReq::get('page') : 1,
        )
    ),
    ...
    'suggestionWaitCount'=>array(
        'query'=>array(
            'name'  =>'suggestion',
            'fields'=>'count(*) as countNums',
            'where' =>'re_time = 0 or re_time is null',
            'type'  =>'countNums'
        )
    ),
    ...
    //根据行业分类ID获取分类名称
    'getHYCategoryExtendNameByCategoryid'=>array(
        'query'=>array(
            'name'  => 'company_category_extend as ce',
            'join'  => 'left join hy_category as cd on cd.id = ce.category_id',
            'where' => 'ce.com_id = #id#',
            'order' => 'cd.id asc',
        )
    ),
    ...
    //根据行业分类ID获取分类名称
    'getGWCategoryExtendNameByCategoryid'=>array(
        'query'=>array(
            'name'  => 'user_category_extend as ce',
            'join'  => 'left join gw_category as cd on cd.id = ce.category_id',
            'where' => 'ce.work_id = #id#',
            'order' => 'cd.id asc',
        )
    ),
    ...
    //根据行业分类ID获取分类名称
    'getQ_HYCategoryExtendNameByCategoryid'=>array(
        'query'=>array(
            'name'  => 'exam_question_hy as ce',
            'join'  => 'left join hy_category as cd on cd.id = ce.hy_id',
            'where' => 'ce.q_id = #id#',
            'order' => 'cd.id asc',
        )
    ),
    ...
    //根据行业分类ID获取分类名称
    'getQ_GWCategoryExtendNameByCategoryid'=>array(
        'query'=>array(
            'name'  => 'exam_question_gw as ce',
            'join'  => 'left join gw_category as cd on cd.id = ce.gw_id',
            'where' => 'ce.q_id = #id#',
            'order' => 'cd.id asc',
        )
    ),
    ...
    'getWorkCount'=>array(
        'query'=>array(
            'name'  =>'user_company',
            'fields'=>'count(*) as amount',
            'where' =>'is_accept = #is_accept# and is_del = 0',
            'type'  =>'amount'
        )
    ),
    ...
    'examRecodCount'=>array(
        'query'=>array(
            'name'  =>'exam_paper_member_record',
            'fields'=>'count(*) as amount',
            'where' =>'did = 1',
            'type'  =>'amount'
        )
    ...
    'examQuestionCount'=>array(
        'query'=>array(
            'name'  =>'exam_paper_member_data',
            'fields'=>'count(*) as amount',
            'where' =>'isright = #isright# ',
            'type'  =>'amount'
        )
    ),

    'examAllQuestionCount'=>array(
        'query'=>array(
            'name'  =>'exam_paper_member_data',
            'fields'=>'count(*) as amount',
            'type'  =>'amount'
    ...
    //系统-最新10条
    'getNewsWorkList'=>array(
        'query'=>array(
            'name'  =>'user_company as uc',
            'join'  =>'left join company as c on c.id = uc.com_id left join member as m on m.user_id = uc.user_id ',
            'fields'=>'m.true_name,m.mobile,c.name,uc.create_time',
            'where' =>'uc.is_accept < 1 and uc.is_del = 0 and (1=#a_level# or c.area=#a_area#)',
            'order' =>'uc.id desc',
            'limit' =>10
        )
    ),

    'getQuestionPoolAll'=>array(
        'query'=>array(
            'name'  => 'exam_pool',
            'order' => 'sort asc',
        )
    ...
    'getPaperTypeAll'=>array(
        'query'=>array(
            'name'  => 'exam_paper_type',
            'order' => 'sort asc,id DESC',
        )
    ),
    ...
    //系统-最新10条
    'getNewsCompanyList'=>array(
        'query'=>array(
            'name'  =>'company as c',
            'fields'=>'c.*',
            'where' =>'c.is_lock = 1 and c.is_del = 0 and (1=#a_level# or c.area=#a_area#)',
            'order' =>'c.id desc',
            'limit' =>10
        )
    ),
